    ...
    public class MedicalRecordsRepository : BaseRepository<MedicalRecords, int>, IMedicalRecordsRepository
    {
        ...
        public async Task<List<OperationResult>> GetMedicalRecordsByDateRangeAsync(DateTime startDate, DateTime endDate)
        {
            ...
            try
            {
                ...
                // Validar existencia de la entidad MedicalRecords en el modelo 
                var entityTypes = _context.Model.GetEntityTypes().Select(e => e.ClrType).ToList();
                ...
                // Consultar los registros médicos dentro del rango de fechas 
                var querys = await (from MedicalRecords in _context.MedicalRecords
                ...
                // Validar si no se encuentran registros médicos 
                if (querys == null || querys.Count == 0)
                {
                    results.Add(new OperationResult
                    {
            ...
            catch (DbUpdateException ex)
            {
                var errorMessage = _configuration["ErrorMedicalRecordsRepository:GetMedicalRecordsByDateRange"]
                                   ?? "Error de actualización al obtener los registros médicos por rango de fechas.";
                _logger.LogError("{ErrorMessage} - DbUpdateException: {Exception}", errorMessage, ex);
                results.Add(new OperationResult
                {
                    Success = false,
            ...
            {
                var errorMessage = "Error de operación inválida al obtener los registros médicos por rango de fechas.";
                _logger.LogError("{ErrorMessage} - InvalidOperationException: {Exception}", errorMessage, ex);
                results.Add(new OperationResult
                {
                    Success = false,
